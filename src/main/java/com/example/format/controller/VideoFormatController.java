...
////    }
    ...
    public String test1(HttpServletRequest servletRequest) throws FileUploadException, IOException {
        DiskFileItemFactory factory = new DiskFileItemFactory();//获得磁盘文件条目工厂
        factory.setSizeThreshold(3000 * 1024); //文件小于3M时放入内存，大于时写入硬盘中
        ServletFileUpload upload = new ServletFileUpload(factory);
        upload.setHeaderEncoding("UTF-8");
        upload.setFileSizeMax(1000 * 1024 * 1024); //设置最大上传附件为1000M
        //实现多附件上传
        MultipartEntityBuilder multipartEntityBuilder =  MultipartEntityBuilder.create();
        multipartEntityBuilder.setMode(HttpMultipartMode.BROWSER_COMPATIBLE);

        List<FileItem> listFileItems = upload.parseRequest(servletRequest);
        for (FileItem listFileItem : listFileItems) {
            if (listFileItem.isFormField()) {
                System.out.println(listFileItem.getFieldName() + "    ====   " + listFileItem.getString());
                multipartEntityBuilder.addTextBody(listFileItem.getFieldName(),listFileItem.getString());

            }else{

                /* example for adding an image part */
                FileBody fileBody = new FileBody(new File(listFileItem.)); //image should be a String
                builder.addPart("media", fileBody);
            }
        }
        File file = new File("F:/gu.amr");
        ...
        HttpPost post = new HttpPost("https://oapi.dingtalk.com/media/upload?access_token=7da03e6d174434a090bec30e28e0e9ad");
